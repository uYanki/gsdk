...
#define LOG_LOCAL_TAG   "arm_dwt"
#define LOG_LOCAL_LEVEL LOG_LEVEL_INFO

// clang-format off

// Debug Exception and Monitor Control Register
#define DEM_REG_CR           *(volatile uint32_t*)0xE000EDFC
#define DEM_MSK_CR_TRCENA    BV(24)

// Debug Watchpoint and Trace (DWT) unit
#define DWT_REG_BASE         0xE0001000
// Control Register
#define DWT_REG_CR           *(volatile uint32_t*)(DWT_REG_BASE + 0x00)
#define DWT_MSK_CR_CYCCNTENA BV(0)
// Cycle Count register
#define DWT_REG_CYCCNT       *(volatile uint32_t*)(DWT_REG_BASE + 0x04)
...
{
    // us to tick
    DWT_DelayTick(u32TimeUs * (SystemCoreClock / 1e6));
}

void DWT_DelayTick(uint32_t u32Ticks)
{
    uint32_t nTickStart = DWT_REG_CYCCNT;
    uint32_t nTickEnd   = nTickStart + u32Ticks;
...
uint32_t DWT_TickToUs(uint32_t u32TickStart, uint32_t u32TickEnd)
{
    ...
    if (u32TickEnd > u32TickStart)
    {
        nTickDelta = u32TickEnd - u32TickStart;
    }
    else
    {
        nTickDelta = U32_MAX - (u32TickStart - u32TickEnd) + 1;
    }
...
{
    // 示波器看多少个 ticks 对应 1us, 用于实现精确微秒延迟

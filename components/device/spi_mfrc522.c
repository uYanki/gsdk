...
uint8_t RC522_Request(spi_rc522_t* pHandle, uint8_t u8ReqCode, uint8_t* pu8TagType)
{
    ...
    ERROR_CHECK_RETURN(RC522_ComMF522(pHandle, CMD_TRANSCEIVE, au8Buffer, 1, au8Buffer, &u16Length));  // 寻卡

    if (u16Length != 0x10)
    {
        return MakeError(ERR_NOT_EXIST, "card doesn't exist");
    }
...
uint8_t RC522_Anticoll(spi_rc522_t* pHandle, uint8_t* pu8Snr)
{
    ...
    ERROR_CHECK_RETURN(RC522_ComMF522(pHandle, CMD_TRANSCEIVE, au8Buffer, 2, au8Buffer, &u16Length));  // 与卡片通信
    ...
    {
        return MakeError(ERR_INVALID_CRC, "fail to check snr");
    }
...
uint8_t RC522_Select(spi_rc522_t* pHandle, uint8_t* pu8Snr)
{
    ...
    ERROR_CHECK_RETURN(RC522_ComMF522(pHandle, CMD_TRANSCEIVE, au8Buffer, 9, au8Buffer, &u16Length));
...
static uint8_t CompareID(uint8_t* au8CardID, uint8_t* au8CompareID)
{
    ...
    for (i = 0; i < 5; ++i)
    {
        ...
        {
            return MakeError(ERR_MISMACTH, "id mismatch");
        }
    }
...
void RC522_ReadWriteTest(spi_rc522_t* pHandle)
{
    ...
    ERROR_CHECK_RETURN_NULL(RC522_Request(pHandle, PICC_REQALL, g_ucTempbuf));
    ERROR_CHECK_RETURN_NULL(RC522_Anticoll(pHandle, au8SerNum));
    ERROR_CHECK_RETURN_NULL(RC522_Select(pHandle, au8SerNum));
    ERROR_CHECK_RETURN_NULL(RC522_AuthState(pHandle, PICC_AUTHENT1A, u8Addr, defaultKeyA, au8SerNum));

    ERROR_CHECK_RETURN_NULL(RC522_Read(pHandle, u8Addr, au8Data));  // 读取
    hexdump(au8Data, ARRAY_SIZE(au8Data), 16, 1, false, "RD: ", 0);
    for (uint8_t i = 0; i < 16; ++i) { ++au8Data[i]; }
    ERROR_CHECK_RETURN_NULL(RC522_Write(pHandle, u8Addr, au8Data));  // 写入
    ERROR_CHECK_RETURN_NULL(RC522_Read(pHandle, u8Addr, au8Data));   // 读取

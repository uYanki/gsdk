...
typedef enum {
    MCP_ERROR_OK        = 0,
    MCP_ERROR_FAIL      = 1,
    MCP_ERROR_ALLTXBUSY = 2,
    MCP_ERROR_FAILINIT  = 3,
    MCP_ERROR_FAILTX    = 4,
    MCP_ERROR_NOMSG     = 5
} MCP_ERROR;
...
static const uint8_t EFLG_MCP_ERRORMASK = EFLG_RX1OVR | EFLG_RX0OVR | EFLG_TXBO | EFLG_TXEP | EFLG_RXEP;
...
void      MCP2515_StartSPI(spi_mcp2515_t* pHandle);
void      MCP2515_EndSPI(spi_mcp2515_t* pHandle);
...
MCP_ERROR MCP2515_SetBitrate(spi_mcp2515_t* pHandle, const CAN_SPEED canSpeed, const CAN_CLOCK canClock);
MCP_ERROR MCP2515_SetFilterMask(spi_mcp2515_t* pHandle, const MASK num, const bool ext, const uint32_t ulData);

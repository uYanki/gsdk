...
static err_t LM75_WriteWord(i2c_lm75_t* pHandle, uint8_t u8MemAddr, uint16_t u16Data)
{
...
static err_t LM75_ReadWord(i2c_lm75_t* pHandle, uint8_t u8MemAddr, uint16_t* pu16Data)
{
...
static err_t LM75_WriteBits(i2c_lm75_t* pHandle, uint16_t u16MemAddr, uint8_t u8StartBit, uint8_t u8BitsCount, uint8_t u8BitsValue)
{
    return I2C_Master_WriteByteBits(pHandle->hI2C, pHandle->u8SlvAddr, u16MemAddr, u8StartBit, u8BitsCount, u8BitsValue, I2C_FLAG_7BIT_SLVADDR | I2C_FLAG_8BIT_MEMADDR);
}

static err_t LM75_ReadBits(i2c_lm75_t* pHandle, uint16_t u16MemAddr, uint8_t u8StartBit, uint8_t u8BitsCount, uint8_t* pu8BitsValue)
{
    return I2C_Master_ReadByteBits(pHandle->hI2C, pHandle->u8SlvAddr, u16MemAddr, u8StartBit, u8BitsCount, pu8BitsValue, I2C_FLAG_7BIT_SLVADDR | I2C_FLAG_8BIT_MEMADDR);
}

static err_t LM75_GetLimit(i2c_lm75_t* pHandle, uint8_t u8MemAddr, float32_t* pf32Value)
{
    uint16_t u16Data;
...
static err_t LM75_SetLimit(i2c_lm75_t* pHandle, uint8_t u8MemAddr, float32_t f32Value)
{
    uint16_t u16Data;
...
err_t LM75_Init(i2c_lm75_t* pHandle)
{
...
err_t LM75_ReadTemp(i2c_lm75_t* pHandle, float32_t* pTemperature)
{
    uint16_t u16Data;
...
err_t LM75_SetHysteresis(i2c_lm75_t* pHandle, float32_t f32Hysteresis)
{
    return LM75_SetLimit(pHandle, LM75_REG_THYST, f32Hysteresis);
}

err_t LM75_GetHysteresis(i2c_lm75_t* pHandle, float32_t* pf32Hysteresis)
{
    return LM75_GetLimit(pHandle, LM75_REG_THYST, pf32Hysteresis);
}

err_t LM75_SetOverTempThold(i2c_lm75_t* pHandle, float32_t f32Threshold)
{
    return LM75_SetLimit(pHandle, LM75_REG_TOS, f32Threshold);
}

err_t LM75_GetOverTempThold(i2c_lm75_t* pHandle, float32_t* pf32Threshold)
{
    return LM75_GetLimit(pHandle, LM75_REG_TOS, pf32Threshold);
}

err_t LM75_SetDeviceMode(i2c_lm75_t* pHandle, lm75_device_mode_e eMode)
{
    return LM75_WriteBits(pHandle, LM75_REG_CONF, 0, 1, eMode);
}

err_t LM75_GetDeviceMode(i2c_lm75_t* pHandle, lm75_device_mode_e* peMode)
{
    return LM75_ReadBits(pHandle, LM75_REG_CONF, 0, 1, peMode);
}

err_t LM75_SetOperMode(i2c_lm75_t* pHandle, lm75_os_operation_mode_e eMode)
{
    return LM75_WriteBits(pHandle, LM75_REG_CONF, 1, 1, eMode);
}

err_t LM75_GetOperMode(i2c_lm75_t* pHandle, lm75_os_operation_mode_e* peMode)
{
    return LM75_ReadBits(pHandle, LM75_REG_CONF, 1, 1, peMode);
}

err_t LM75_SetOutputSignalPolarity(i2c_lm75_t* pHandle, lm75_os_polarity_e ePolarity)
{
    return LM75_WriteBits(pHandle, LM75_REG_CONF, 2, 1, ePolarity);
}

err_t LM75_GetOutputSignalPolarity(i2c_lm75_t* pHandle, lm75_os_polarity_e* pePolarity)
{
    return LM75_ReadBits(pHandle, LM75_REG_CONF, 2, 1, pePolarity);
}

err_t LM75_SetFaultQueue(i2c_lm75_t* pHandle, lm75_fault_queue_e eFaultQueue)
{
    return LM75_WriteBits(pHandle, LM75_REG_CONF, 3, 2, eFaultQueue);
}

err_t LM75_GetFaultQueue(i2c_lm75_t* pHandle, lm75_fault_queue_e* peFaultQueue)
{

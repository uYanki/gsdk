...
{
    /* Set memory */
    memset(pHandle->pu8Buffer, (eColor == BLACK) ? 0x00 : 0xFF, sizeof(pHandle->pu8Buffer));
}
...
char Framebuf_Putc(framebuf_t* pHandle, char ch, oled_font_t* Font, color_e eColor)
{
    ...
    if (pHandle->u16Width <= (pHandle->u16CurrentX + Font->width) ||
        pHandle->u16Height <= (pHandle->u16CurrentY + Font->height))
    {
        /* Error */
        return 0;
    }
    ...
    for (i = 0; i < Font->height; i++)
    {
        ...
        for (j = 0; j < Font->width; j++)
        {
            if ((b << j) & 0x8000)
            {
                Framebuf_DrawPixel(pHandle, pHandle->u16CurrentX + j, (pHandle->u16CurrentY + i), (color_e)eColor);
            }
            else
            {
                Framebuf_DrawPixel(pHandle, pHandle->u16CurrentX + j, (pHandle->u16CurrentY + i), (color_e)!eColor);
    ...
    /* Increase pointer */
    pHandle->u16CurrentX += Font->width;

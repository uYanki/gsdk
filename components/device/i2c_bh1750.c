...
{
    return I2C_Master_TransmitByte(pHandle->hI2C, pHandle->u8SlvAddr, u8Cmd, I2C_FLAG_7BIT_SLVADDR);
}
...
{
    return I2C_Master_ReceiveWord(pHandle->hI2C, pHandle->u8SlvAddr, pu16Data, I2C_FLAG_7BIT_SLVADDR | I2C_FLAG_WORD_BIG_ENDIAN);
}

err_t BH1750_Init(i2c_bh1750_t* pHandle)
{
...
err_t BH1750_PowerOn(i2c_bh1750_t* pHandle)
{
    return BH1750_WriteCmd(pHandle, CMD_POWER_ON);
}

err_t BH1750_PowerOff(i2c_bh1750_t* pHandle)
{
    return BH1750_WriteCmd(pHandle, CMD_POWER_DOWN);
}

err_t BH1750_Reset(i2c_bh1750_t* pHandle)
{
 ...
 */
err_t BH1750_SetSensitivity(i2c_bh1750_t* pHandle, uint8_t u8Sensitivity)
{
 ...
 * @brief 读取光强度
 */
err_t BH1750_GetLux(i2c_bh1750_t* pHandle, float32_t* pf32Lux)
{
    uint16_t u16Data;

...
struct flexbtn {
    __IN uint8_t u8ID; /* button id */

    __IN uint16_t u16ShortPressStartTick;       /* short press start time */
    __IN uint16_t u16LongPressStartTick;        /* long press start time */
    __IN uint16_t u16LongHoldStartTick;         /* long hold start time */
    __IN uint16_t u16MaxMultipleClicksInterval; /* multiple clicks interval */

    __IMPL void (*pfnEventCb)(flexbtn_t* pHandle, flexbtn_event_e eEvent);
    __IMPL bool (*pfnIsPressed)(flexbtn_t* pHandle); /* get button state, true:pressed, false:released */
 ...
 */
bool FlexBtn_Attach(flexbtn_t* pHandle);

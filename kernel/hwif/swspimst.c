...
static err_t SWSPI_Master_Init(spi_mst_t* pHandle, uint32_t u32ClockSpeedHz, spi_duty_cycle_e eDutyCycle, uint16_t u16Flags)
{
    ...
    ERROR_CHECK_RETURN(SWSPI_Master_SetClock(pHandle, u32ClockSpeedHz, eDutyCycle));
    ...
    switch (pHandle->u16TimingConfig & SPI_FLAG_DATAWIDTH_Msk)
    {
        ...
        case SPI_FLAG_DATAWIDTH_16B:
        {
            ...
            while (u16Size--)
            {
                ...
                memcpy((void*)*pu8RxData, (void*)&u16RxData, sizeof(uint16_t));
        ...
        case SPI_FLAG_DATAWIDTH_32B:
        {
            ...
            while (u16Size--)
            {
                ...
                memcpy((void*)*pu8RxData, (void*)&u32RxData, sizeof(uint32_t));

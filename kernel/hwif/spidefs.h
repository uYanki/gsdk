...
typedef struct spimst {
    ...
    uint16_t u16TimingConfig;

#if CONFIG_SWSPI_MODULE_SW

    uint32_t u32FirstCycleUs;   // [swspi only] first edge duration @SPI_FLAG_FAST_CLOCK_DISABLE
    uint32_t u32SecondCycleUs;  // [swspi only] second edge duration @SPI_FLAG_FAST_CLOCK_DISABLE

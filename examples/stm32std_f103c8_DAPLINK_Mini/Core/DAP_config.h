...
#elif  defined(UIDISK_V1)

#define USART_CLOCK(state)		RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART1, state)
#define USART_GPIO_CLOCK(state) RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB, state)
#define USART_REMAP()			GPIO_PinRemapConfig(GPIO_Remap_USART1, ENABLE)
...
// USB Connect Pull-Up
 ...
 ||   defined ( BOARD_V3 )
	...
	#define PIN_SWDIO_TMS_PIN		8

	// SWCLK/TCK Pin
	#define PIN_SWCLK_TCK_PORT		GPIOB
	#define PIN_SWCLK_TCK_PIN		9
...
#elif defined ( STLINK_V20 ) || defined ( STLINK_V21 )
...
// Debug Unit LEDs
	...
	#define LED_CONNECTED_RCC		RCC_APB2ENR_IOPCEN
...
#define PIN_TDO					PIN_MASK(PIN_TDO_PIN)
#define PIN_TDI					PIN_MASK(PIN_TDI_PIN)
	...
	#define PIN_nRESET_HIGH()							\
		do {											\
			...
			PIN_nRESET_PORT->CRH = (PIN_nRESET_PORT->CRH & ~PIN_MODE_MASK(PIN_nRESET_PIN - 8))	\
				| PIN_MODE((GPIO_Mode_IN_FLOATING & 0x0F), PIN_nRESET_PIN - 8);	\
		} while (0)
	...
	#define PIN_nRESET_HIGH()							\
		do {											\
			PIN_nRESET_PORT->BSRR = PIN_nRESET;			\
			PIN_nRESET_PORT->CRL = (PIN_nRESET_PORT->CRL & ~PIN_MODE_MASK(PIN_nRESET_PIN))	\
				| PIN_MODE((GPIO_Mode_IN_FLOATING & 0x0F), PIN_nRESET_PIN); \
		} while (0)
...
//	without GPIO_Init call
	...
	#define PIN_SWDIO_TMS_OUT_DISABLE()					\
		do {											\
			...
			PIN_SWDIO_TMS_PORT->CRH = (PIN_SWDIO_TMS_PORT->CRH & ~PIN_MODE_MASK(PIN_SWDIO_TMS_PIN - 8)) \
				| PIN_MODE((GPIO_Mode_IPU & 0x0F), PIN_SWDIO_TMS_PIN - 8);	\
		} while (0)
	...
	#define PIN_SWDIO_TMS_OUT_ENABLE()					\
		do {											\
			...
			PIN_SWDIO_TMS_PORT->CRH = (PIN_SWDIO_TMS_PORT->CRH & ~PIN_MODE_MASK(PIN_SWDIO_TMS_PIN - 8)) \
				| PIN_MODE(((GPIO_Mode_Out_PP | GPIO_Speed_50MHz) & 0x0F), PIN_SWDIO_TMS_PIN - 8);	\
		} while (0)
	...
	#define PIN_SWDIO_TMS_OUT_DISABLE()					\
		do {											\
			PIN_SWDIO_TMS_PORT->BSRR = PIN_SWDIO_TMS;	\
			PIN_SWDIO_TMS_PORT->CRL = (PIN_SWDIO_TMS_PORT->CRL & ~PIN_MODE_MASK(PIN_SWDIO_TMS_PIN)) \
				| PIN_MODE((GPIO_Mode_IPU & 0x0F), PIN_SWDIO_TMS_PIN);	\
		} while (0)
	...
	#define PIN_SWDIO_TMS_OUT_ENABLE()					\
		do {											\
			PIN_SWDIO_TMS_PORT->BRR  = PIN_SWDIO_TMS;	\
			PIN_SWDIO_TMS_PORT->CRL = (PIN_SWDIO_TMS_PORT->CRL & ~PIN_MODE_MASK(PIN_SWDIO_TMS_PIN)) \
				| PIN_MODE(((GPIO_Mode_Out_PP | GPIO_Speed_50MHz) & 0x0F), PIN_SWDIO_TMS_PIN);	\
		} while (0)

...
static inline s16 __get_s16(s16* p) { return *p; }
static inline s32 __get_s32(s32* p) { return (s32)he32((const u8*)p); }
static inline s64 __get_s64(s64* p) { return (s64)he64((const u8*)p); }
static inline u16 __get_u16(u16* p) { return *p; }
static inline u32 __get_u32(u32* p) { return (u32)he32((const u8*)p); }
static inline u64 __get_u64(u64* p) { return (u64)he64((const u8*)p); }

static inline void __set_s16(s16* p, s16 x) { *p = x; }
static inline void __set_s32(s32* p, s32 x) { memcpy((u8*)p, &x, sizeof(x)); }
static inline void __set_s64(s64* p, s64 x) { memcpy((u8*)p, &x, sizeof(x)); }
static inline void __set_u16(u16* p, u16 x) { *p = x; }
static inline void __set_u32(u32* p, u32 x) { memcpy((u8*)p, &x, sizeof(x)); }
static inline void __set_u64(u64* p, u64 x) { memcpy((u8*)p, &x, sizeof(x)); }

static inline void __inc_s16(s16* p, s16 d) { *p += d; }
static inline void __inc_s32(s32* p, s32 d) { __set_s32(p, __get_s32(p) + d); }
static inline void __inc_s64(s64* p, s64 d) { __set_s64(p, __get_s64(p) + d); }
static inline void __inc_u16(u16* p, u16 d) { *p += d; }
static inline void __inc_u32(u32* p, u32 d) { __set_u32(p, __get_u32(p) + d); }
static inline void __inc_u64(u64* p, u64 d) { __set_u64(p, __get_u64(p) + d); }
...
typedef struct __packed {
    ...
    u16 _Resv610;                        // P0610
    u16 u16OpenPeriod;                   // P0611 开环测试.运行周期
    u16 u16OpenElecAngInit;              // P0612 开环测试.电角度初值
    s16 s16OpenElecAngInc;               // P0613 开环测试.电角度自增量
    s16 s16OpenUqRef;                    // P0614 开环测试.D轴指令
    s16 s16OpenUdRef;                    // P0615 开环测试.Q轴指令
    u16 _Resv616;                        // P0616
    u16 u16MotEncIdentEn;                // P0617 电机编码器辨识使能
    u16 u16MotEncIdentDirMatched;        // P0618 电机编码器辨识.方向匹配状态
    u16 u16MotEncIdentPolePairs;         // P0619 电机编码器辨识.电机极对数
    u32 u16MotEncIdentOffset;            // P0620 电机编码器辨识.编码器安装偏置角
    u32 u16MotEncIdentRes;               // P0622 电机编码器辨识.编码器分辨率
    u16 u16MotEncIdentState;             // P0624 电机编码器辨识.运行状态
    u16 u16MotEncIdentErr;               // P0625 电机编码器辨识.错误类型
    u16 _Resv626;                        // P0626
    u16 _Resv627;                        // P0627
    u16 u16TrqMulRefSel;                 // P0628 多段转矩指令选择
    u16 u16AiTrqCoeff0;                  // P0629 模拟量AI0转矩系数
    u16 u16AiTrqCoeff1;                  // P0630 模拟量AI1转矩系数
    u16 _Resv631;                        // P0631
    u16 u16EncCmd;                       // P0632 编码器指令

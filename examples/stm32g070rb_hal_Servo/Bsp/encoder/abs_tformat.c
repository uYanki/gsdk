...
void cmd_tx(u8* pau8TxBuf, u8 u8TxCnt)
{
    ...
    // HAL_UART_Receive(&UART_TFORMAT, sAbsTformat.au8RxBuf, 6, 0xFF);
    // // DelayBlockUs(1);
    // void TFormatRdPos2(u8 * pau8RxBuf);
    // TFormatRdPos2(sAbsTformat.au8RxBuf);
}

void cmd_rx(u8* pau8RxBuf, u8 u8RxCnt)  // 异步读取
{
...
void TFormatRdPos2(u8* pau8RxBuf)
{
    ...
    if (u8CrcSts == 0)  // success
    {
        ...
        // P(0).s32SpdDigRef05 = pEnc->u32Pos;
        P(0).s32SpdDigRef05 = (((u32)pau8RxBuf[4] << 16) + ((u32)pau8RxBuf[3] << 8) + (u32)pau8RxBuf[2]) >> 2;
        P(0).s32SpdDigRef06++;
...
void TFormatWrCmd2(abs_drv_t* pEnc, abs_cmd_e eCmd)
{

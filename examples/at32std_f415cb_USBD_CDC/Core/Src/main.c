...
static uint8_t m_au8UsbBuff[256];
...
int main(void)
{
    ...
    uint16_t u16RxDatLen;
    uint16_t u16Timeout;

    while (1)
    {
        // 读取虚拟串口里的数据
        u16RxDatLen = usb_vcp_get_rxdata(&otg_core_struct.dev, m_au8UsbBuff);
        ...
        if (u16RxDatLen > 0)
        {
            ...
            {
                // 发送接收到的数据
                if (usb_vcp_send_data(&otg_core_struct.dev, m_au8UsbBuff, u16RxDatLen) == SUCCESS)
                {
                    break;
                }
...
static void USBD_Init(void)
{
    ...
    {  // usb
        ...
        usb_sts_type sts = usbd_init(&otg_core_struct, USB_FULL_SPEED_CORE_ID, USB_ID, &cdc_class_handler, &cdc_desc_handler);

...
int main(void)
{
     ...
     */
    while ((u8RxDat = au_baud_rate_detection()) == 0);
    ...
    while (*szAck)
    {
        while (usart_flag_get(AU_UART, USART_TDC_FLAG) == RESET);
        usart_data_transmit(AU_UART, (uint16_t)(*szAck++));
    }

    while (1)  // 4800 ~ 115200 ok
    {
        if (usart_flag_get(AU_UART, USART_RDBF_FLAG) != RESET)
        {
            u8RxDat = usart_data_receive(AU_UART);
            bRxSts  = true;
        }
        if (bRxSts == true)
        {
            ...
            {
                usart_data_transmit(AU_UART, (uint16_t)u8RxDat);
                bRxSts = false;
            }
        }
    }
